...
class SignatureValidatorController(http.Controller):
    ...
    def _validate_p12_certificate(self, cert_data, password):
        ...
        Incluye verificación de cadena de confianza contra CAs configuradas.
        
        Args:
            cert_data: Datos binarios del archivo P12
        ...
        try:
            ...
            # El certificado es válido si:
            # 1. Está dentro de su período de validez
            # 2. Si hay CAs configuradas, debe verificar contra alguna
            ...
            if has_cas_configured:
                ...
                is_valid = is_time_valid  # Sin CAs, solo verificar fechas
            ...
            if 'password' in error_msg or 'mac' in error_msg or 'decrypt' in error_msg:
                _logger.warning(f"Contraseña incorrecta para certificado P12")
                return {
                    'success': False,
                    'password_correct': False,
                    'error': 'La contraseña proporcionada es incorrecta.',
                    'validation_errors': [
                        'La contraseña del certificado P12 es incorrecta. '
                        'Verifique que está usando la contraseña correcta para este archivo.'
                    ]
                }
            _logger.error(f"Error de valor en certificado P12: {e}")
            return {
                'success': False,
                'error': f'El archivo P12 no es válido: {str(e)}',
                'validation_errors': [
                    f'No se pudo procesar el archivo P12: {str(e)}. '
                    'Asegúrese de que el archivo no está corrupto y es un certificado P12/PFX válido.'
                ]
            }
        except Exception as e:
            _logger.error(f"Error procesando certificado P12: {e}")
            return {
                'success': False,
                'error': f'Error al procesar el certificado: {str(e)}',
                'validation_errors': [f'Error inesperado al procesar el certificado: {str(e)}']
            }
