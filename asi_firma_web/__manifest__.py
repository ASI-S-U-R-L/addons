{'name': 'ASI Firma Web', 'summary': 'Firmar documentos PDF desde el sitio web usando asi_pdf_signature',
  'version': '16.0.1.2.0',
    'category': 'Website',
    'author': 'Jose L. Reyes Alvarez',
  'depends': ['website', 'asi_pdf_signature', 'asi_signature_workflow'],
  'data': [
      'data/website_menu.xml',
      'views/templates.xml',
      'views/solicitud_firma_views.xml',
      'views/menus.xml',
  ],
  'assets': {
    'web.assets_frontend': [
      'asi_firma_web/static/src/css/drag_drop.css',
      'asi_firma_web/static/src/js/drag_drop.js',
      'asi_firma_web/static/src/css/bandeja_split.css',
      'asi_firma_web/static/src/js/bandeja_split.js',
    ],
  },
  'license': 'LGPL-3',
    'images': ['static/description/icon.png'],
    'installable': True,
    'application': False}
